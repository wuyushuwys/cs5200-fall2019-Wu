...
# Part 1: developers and users table
...
VALUES
	(123, 'Home', 'Landing page', (SELECT `id` FROM `website` WHERE `name`='CNET'), '2019-09-04', CURDATE(), 123434),
	(234, 'About', 'Website description', (SELECT `id` FROM `website` WHERE `name`='Gizmodo'), '2019-09-04', CURDATE(), 234545),
	(345, 'Contact', 'Addresses, phones, and contact info', (SELECT `id` FROM `website` WHERE `name`='Wikipedia'), '2019-09-04', CURDATE(), 345656),
	(456, 'Preferences', 'Where users can configure their preferences', (SELECT `id` FROM `website` WHERE `name`='CNN'), '2019-09-04', CURDATE(), 456776),
	(567, 'Profile', 'Users can configure their personal information', (SELECT `id` FROM `website` WHERE `name`='CNET'), '2019-09-04', CURDATE(), 567878);

# create page_role info
SET SQL_SAFE_UPDATES=0;
...
# Part 5 insert phones and addresses for users and developers
