...
//        create developer and users
        ...
        developerDao.createUser(dan);
        developerDao.createUser(ed);
...
//        create pages
        ...
        Page home = new Page(123,"Home","Landing page",Date.valueOf("2019-09-04"),Date.valueOf("2019-10-20"),123434);
        Page about = new Page(234,"About","Website description",Date.valueOf("2019-09-04"),Date.valueOf("2019-10-20"),234545);
        Page contact = new Page(345,"Contact","Addresses, phones, and contact info",Date.valueOf("2019-09-04"),Date.valueOf("2019-10-20"),345656);
        Page preferences = new Page(456,"Preferences","Where users can configure preferences their",Date.valueOf("2019-09-04"),Date.valueOf("2019-10-20"),456776);
        Page profile = new Page(567,"Profile","Users can configure their personal information",Date.valueOf("2019-09-04"),Date.valueOf("2019-10-20"),567878);
...
//    public Widget(int id, String name, int width, int height, String cssClass, String cssStyle, String text, int order, int size, String html, String src, String url, boolean shareable, boolean expandable, type dtype) {
...
//        Update widget - Update the relative order of widget head345 on the page so that it's new order is 3. Note that the other widget's order needs to update as well
        ...
        Collection<Widget> widgets = widgetDao.findWidgetsForPage(head345.getPage_id());
...
//        Update roles - Swap Charlie's and Bob's role in CNET's Home page
        ...
        Role bob_cnet_role = page_roleDao.findPageRoleForDeveloper(bob.getId(), cnet_home.getId());
        Role charlie_cnet_role = page_roleDao.findPageRoleForDeveloper(charlie.getId(), cnet_home.getId());
        String bob_role = bob_cnet_role.getRole();
        String charlie_role = charlie_cnet_role.getRole();
...
//    Delete page - Remove the last updated page in Wikipedia
        Date max_date = Date.valueOf("1900-01-01");
        for (Page page : pageDao.findPagesForWebsite(wikipedia.getId())){
            int status=max_date.compareTo(page.getUpdated());
            if(status<0){
                max_date = page.getUpdated();
                max_id = page.getId();
            }
        }
        pageDao.deletePage(max_id);
//        Delete website - Remove the CNET website, as well as all related roles and privileges relating developers to the Website and Pages
